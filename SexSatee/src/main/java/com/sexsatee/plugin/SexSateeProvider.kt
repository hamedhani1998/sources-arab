...
class SexSateeProvider : MainAPI() {
    override var name = "سكساتي"
    override var mainUrl = "https://sex-satee.com"
    ...
    override val mainPage = mainPageOf(
        ...
        "category/سكس-عراقي/" to "سكس عراقي",
    )
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse? {
        return try {
            ...
            val items = doc.select("article.thumb-block, article.loop-video").mapNotNull { item ->
                try {
                    val a = item.selectFirst("a") ?: return@mapNotNull null
                    val href = a.attr("href") ?: return@mapNotNull null
                    val title = item.selectFirst("header.entry-header span")?.text()?.trim()
                    ...
                    val rating = item.selectFirst("div.rating-bar-meter")?.attr("style")?.let {
                        Regex("""width:\s*(\d+)%""").find(it)?.groupValues?.get(1)
                    }
    ...
    override suspend fun search(query: String): List<SearchResponse>? {
        return try {
            ...
            doc.select("article.thumb-block, article.loop-video").mapNotNull { item ->
                try {
                    val a = item.selectFirst("a") ?: return@mapNotNull null
                    val href = a.attr("href") ?: return@mapNotNull null
                    val title = item.selectFirst("header.entry-header span")?.text()?.trim()
